...
hook.Add("DrawHUD", "nametags", function()
    ...
    if trace:IsValid() then
        ...
        if typex(trace) == "player" and (trace:GetHealth() / trace:GetMaxHealth() * 100) > 0 then
            ...
            graphics.DrawText(trace:GetNickname() .. " " .. math.round(trace:GetHealth() / trace:GetMaxHealth() * 100) .. "%", Vec2(scrw / 2, scrh / 2 + 100), "segoeui.ttf", Vec2() + 14, Color(1, 1, 1, 1), TEXT_OFFSET, Vec2() + 1)
        end
    end
	...
	for key, ent in pairs(entities.FindInSphere(me:GetPos(), RANGE)) do
		if typex(ent) == "player" and me ~= ent then
            ...
            if vis > 0 then
                ...
                graphics.DrawText(ent:GetNickname(), pos, "segoeui.ttf", Vec2() + 18, Color(1, 1, 1, alpha), TEXT_OFFSET, Vec2() + 1)
				
                if ent:GetHealth() > 0 and ent:GetHealth() < ent:GetMaxHealth() then					
					graphics.DrawRect(Rect(pos.x - 52, pos.y + 25, HEALTH_BAR_WIDTH, HEALTH_BAR_HEIGHT), Color(0.5, 0.5, 0.5, alpha), nil, 1, Color(0, 0, 0, alpha))
