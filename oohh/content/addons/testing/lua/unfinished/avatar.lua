...
urlmat.ActiveView = urlmat.ActiveView or NULL
urlmat.Queue = urlmat.Queue or {}
urlmat.Cache = urlmat.Cache or {}

if urlmat.ActiveView:IsValid() then
	urlmat.ActiveView:Remove()
end
...
function urlmat.Think()
	if table.count(urlmat.Queue) > 0 then
		for url, data in RandomPairs(urlmat.Queue) do
			...
			if not urlmat.ActiveView:IsValid() then
...
function urlmat.StartDownload(url, data)
	...
	local web = WebView(urlmat.TextureSize, urlmat.TextureSize)
