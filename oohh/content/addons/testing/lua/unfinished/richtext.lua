...
function RichObject:BuildWidthCache(font)
	...
	if not CharWidth[font] then
		...
		for i=1,127 do
			...
			CharWidth[font][c] = surface.GetTextSize(fonts[font], c)
		end
...
function RichObject:CalcTextSizeEx(buffer, font)
	...
	return surface.GetTextSize(font, buff)
end
...
function RichObject:BuildStyle(pos, length, color, font, time, onclick, onclickref, filter, newline)
	...
	style.xwidth, style.yheight = self:CalcTextSizeEx(buffer, fonts[font])
...
function RichObject:Parse()
	...
	for k, v in ipairs(self.styles) do
		...
		--if bit.band(v.filter, self.cfilter) then
...
function RichObject:Draw(fade)
	...
	if not self.drawbounds[1] or self.drawbounds[1] == 0 or not self.drawbounds[2] or self.drawbounds[2] == 0 then return end
	...
	for i=self.drawbounds[1], self.drawbounds[2] do
		...
		if a > 0 then
			fonts[block.font] = fonts[block.font] or Font(block.font)
			surface.SetFont(fonts[block.font])
