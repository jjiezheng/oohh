...
function PANEL:OnThink()
	if self.nextlayout and CurTime() >= self.nextlayout then
		self:DoLayout()
		return
	end
	if not self.nextupdate then self.nextupdate = CurTime() + 6 end
	if self.layoutfinished and CurTime() > self.nextupdate then
		self:DoLayout()
		self.nextupdate = CurTime() + 6
		return
	end
end
...
local testfont = Font("tahoma.ttf")
