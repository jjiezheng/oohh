function aahh.Create(name, parent)
	...
	for key, val in pairs(pnl) do
		if type(val) == "userdata" and getmetatbale(val) and val.Copy then
			pnl[key] = val:Copy()
		end
	end
	...
	pnl.aahh_id = table.insert(aahh.ActivePanels, 1, pnl)
...
function aahh.SafeCall(pnl, name, ...)
	...
	if not pnl[name] then return end
	if type(pnl[name]) ~= "function" then return end
	
	local tbl = {pcall(pnl[name], pnl, ...)}
	local success = tbl[1]
	local err = tbl[2]
...
do -- meta
	...
	aahh.GetSet(PANEL, "TrapInsideParent", true)
	...
	function PANEL:SafeCall(name, ...)
		return aahh.SafeCall(self, name, ...)
	end
	...
	do -- parenting
		...
		function PANEL:UnParent()
			...
			if parent:IsValid() then
				...
				self:SafeCall("OnUnParent", parent)
			end			
		end
	end		
	...
	do -- dock
		...
		function PANEL:DockLayout()			
			...
			local margins = self.Margin or Rect()
			
			local x = margins.x
			local y = margins.y
			local w = self:GetWidth() - x - margins.w
			local h = self:GetHeight() - y - margins.h
	...
	function PANEL:SetVisible(b)
		if b ~= self.Visible then
			if b then
				self:SafeCall("OnShow")
			else
				self:SafeCall("OnHide")
			end
		end
		self.Visible = b
	end
...
--		if self.OnMouseMove then
	...
	do -- events
		...
		function PANEL:Remove()
			hook.Call("OnPanelRemove", self)

			for key, pnl in pairs(self:GetChildren()) do
				pnl:Remove()
			end
			
			self:SafeCall("OnRemove")
