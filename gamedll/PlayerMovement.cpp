...
//-----------------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------------
void CPlayerMovement::ProcessPrediction(CPlayer& player)
{
	const bool isRemoteClient = player.IsRemote();
	const bool isNetJumping = g_pGameCVars->pl_velocityInterpSynchJump && m_movement.jump && isRemoteClient;
	const bool doVelInterpolation = isRemoteClient && player.GetPlayerInput() && !player.IsDead();
	...
	if (doVelInterpolation)
	{
		...
		if (g_pGameCVars->pl_clientInertia >= 0.0f)
		{
			...
			animParams = player.GetAnimatedCharacter()->GetParams();
			animParams.inertia = g_pGameCVars->pl_clientInertia;
			player.SetAnimatedCharacterParams( animParams );
		}
			
		if (isNetJumping)
		{
			m_request.velocity = player.m_jumpVel;
			m_request.type = eCMT_JumpAccumulate;
		}
		else
		{
			CNetPlayerInput *playerInput = (CNetPlayerInput*) player.GetPlayerInput();
			Vec3 interVel, desiredVel; 

			playerInput->GetDesiredVel(player.GetEntity()->GetPos(), desiredVel);
			interVel = desiredVel;

			const bool isPlayerInAir = (player.m_stats.inAir > 0.0f) && (player.m_stats.onGround < 0.01f);
			if (isPlayerInAir)
			{
			...
			m_request.velocity.x = interVel.x;
			m_request.velocity.y = interVel.y;
		}
	}				
}
